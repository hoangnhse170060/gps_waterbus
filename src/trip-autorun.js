...
export function createTripAutorun(ctx) {
  ...
  function tripMissionPublic(mission) {
    ...
    return {
      ...
      takenOverAt: mission.takenOverAt || null,
      routeCode: mission.routeCode,
      status: mission.status,
      handoffMissionType: mission.handoffMissionType || null,
      handoffLat: Number.isFinite(Number(mission.handoffLat)) ? Number(mission.handoffLat) : null,
      handoffLng: Number.isFinite(Number(mission.handoffLng)) ? Number(mission.handoffLng) : null,
  ...
  async function tickOneMission(mission, nowMs) {
    ...
    // Tàu sự cố / bảo trì / SOS kéo → pause trip (giống nhau: không Active thì đứng).
    // Tàu thay sau takeover vẫn chạy (takenOverFrom).
    if (!mission.takenOverFrom && (
      boatIsFrozenForIncident(mission.boatCode)
      || (typeof isActiveBoatCode === 'function' && !isActiveBoatCode(mission.boatCode))
    )) {
