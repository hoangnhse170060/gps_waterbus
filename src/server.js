...
function boatNeedsIncidentFreeze(boatOrCode) {
  ...
  if (code) {
    for (const mission of state.rescueMissions.values()) {
      ...
      // Chỉ neo tàu gặp sự cố / đang bị kéo — không khóa SOS đang chạy tới hiện trường.
...
/** Contract RescueDispatched: loại nhiệm vụ tàu thay khách. */
function normalizeReplacementMissionType(raw) {
  const t = String(raw || '').trim();
  if (!t) return null;
  if (/transferatincident/i.test(t)) return 'TransferAtIncidentLocation';
  if (/continuefromstation/i.test(t)) return 'ContinueFromStation';
  if (/passengerrecovery/i.test(t)) return 'PassengerRecoveryRequired';
  if (/^none$/i.test(t)) return 'None';
  return t;
}

function extractReplacementMissionFields(...sources) {
  const src = Object.assign({}, ...sources.filter((s) => s && typeof s === 'object'));
  const type = normalizeReplacementMissionType(
    src.replacementMissionType || src.ReplacementMissionType,
  );
  const targetLat = Number(
    src.replacementTargetLat ?? src.ReplacementTargetLat
    ?? src.replacementTarget?.lat,
  );
  const targetLng = Number(
    src.replacementTargetLng ?? src.ReplacementTargetLng
    ?? src.replacementTarget?.lng,
  );
  const delayMin = Number(src.replacementDelayMinutes ?? src.ReplacementDelayMinutes);
  const onboard = Number(src.onboardPassengerCount ?? src.OnboardPassengerCount);
  const future = Number(src.futurePassengerCount ?? src.FuturePassengerCount);
  return {
    replacementMissionType: type,
    ...
    replacementEstimatedResumeAt: cleanOptionalText(
      src.replacementEstimatedResumeAt || src.ReplacementEstimatedResumeAt,
    ) || null,
    onboardPassengerCount: Number.isFinite(onboard) ? onboard : null,
    futurePassengerCount: Number.isFinite(future) ? future : null,
  };
}
...
function upsertIncidentRecord(incident, { removeIfResolved = true } = {}) {
  ...
  state.openIncidents.set(incident.incidentId, {
    ...
    replacementEstimatedResumeAt: incident.replacementEstimatedResumeAt ?? prev.replacementEstimatedResumeAt ?? null,
    onboardPassengerCount: incident.onboardPassengerCount ?? prev.onboardPassengerCount ?? null,
...
function snapCoordinatesToRiver(coordinates, { force = false } = {}) {
  ...
  const built = buildRiverPath(start, end, getRescueRiverBasePath(), {
    joinMeters: 90,
    corridorOnly: true,
  });
...
function assignRescueRiverPath(mission, from, to) {
  if (!mission) return null;
  const built = buildRiverPath(from, to, getRescueRiverBasePath(), {
    joinMeters: 90,
    corridorOnly: true,
  });
  ...
  const riverStart = built.coordinates[0];
  if (riverStart) {
    mission.currentLat = riverStart.lat;
    mission.currentLng = riverStart.lng;
  }
...
function startRescueAutomation(incident) {
  ...
  // GPS thật của SOS trước khi seed — dùng để biết đã ở hiện trường hay chưa.
  const rescueHubBefore = state.hubBoats.get(rescueBoatCode);
  const hadRescueGps = Number.isFinite(Number(rescueHubBefore?.lat))
  ...
  const resolved = resolveIncidentTargetCoords({
    ...
    from: Number.isFinite(startLat) ? { lat: startLat, lng: startLng } : null,
  });
  let resolvedTargetLat = resolved.lat;
  let resolvedTargetLng = resolved.lng;
  const riverTarget = projectOnPath(
    getRescueRiverBasePath(),
    { lat: resolvedTargetLat, lng: resolvedTargetLng },
  );
  if (riverTarget) {
    resolvedTargetLat = riverTarget.lat;
    resolvedTargetLng = riverTarget.lng;
  }
  ...
  // Chưa có GPS thật: không seed SOS ngay tại hiện trường (sẽ bị coi là đã tới).
  // Xuất phát từ bến gần hiện trường rồi chạy ra cứu.
  if (!hadRescueGps && Number.isFinite(resolvedTargetLat) && Number.isFinite(resolvedTargetLng)) {
    const berth = nearestStationBeyond(
      { lat: resolvedTargetLat, lng: resolvedTargetLng },
      80,
    ) || nearestStationTo({ lat: resolvedTargetLat, lng: resolvedTargetLng });
    if (berth && Number.isFinite(berth.lat) && Number.isFinite(berth.lng)) {
      const berthDist = distanceMeters(
        { lat: berth.lat, lng: berth.lng },
        { lat: resolvedTargetLat, lng: resolvedTargetLng },
      );
      if (berthDist > Math.max(3, Number(env.RESCUE_ARRIVE_METERS || 15))) {
        startLat = berth.lat;
        startLng = berth.lng;
        const prevHub = state.hubBoats.get(rescueBoatCode) || {};
        state.hubBoats.set(rescueBoatCode, {
          ...prevHub,
          boatCode: rescueBoatCode,
          boatName: rescueBoat?.boatName || prevHub.boatName || rescueBoatCode,
          boatId: rescueBoat?.boatId || prevHub.boatId || null,
          lat: startLat,
          lng: startLng,
          speedKmh: 0,
          ...
          isOnline: true,
          source: 'rescue-seed-berth',
          updatedAt: new Date().toISOString(),
        });
        console.log(
          `[rescue-gps] SOS chưa có GPS → xuất phát bến `
          + `${berth.station?.stationCode || '?'} (${Math.round(berthDist)}m tới hiện trường)`,
        );
      }
    }
  }
  ...
  // Chỉ "đã tại hiện trường" khi SOS CÓ GPS thật và đúng sát chỗ tàu sự cố — không dùng vị trí seed.
  const preSeedDist = hadRescueGps
    ? distanceMeters(
      { lat: Number(rescueHubBefore.lat), lng: Number(rescueHubBefore.lng) },
      { lat: resolvedTargetLat, lng: resolvedTargetLng },
    )
    : Infinity;
  const alreadyAtScene = preSeedDist <= arrivalMeters;
  ...
  if (alreadyAtScene) {
    ...
    if (towDest) {
      ...
      console.log(
        `[rescue-gps] SOS đã sát hiện trường (${Math.round(preSeedDist)}m) `
  ...
  const mission = {
    ...
    replacementBoatCode: replacementBoatCode && replacementBoatCode !== rescueBoatCode
      ? replacementBoatCode
      : null,
    ...extractReplacementMissionFields(incident || {}),
  ...
  if (destinationMeta) {
    ...
    const towHeading = pointAtDistance(mission.pathCoordinates, 0).heading || bearingDegrees(
      { lat: startLat, lng: startLng },
      { lat: missionTargetLat, lng: missionTargetLng },
    ) || 0;
    mission.incidentCurrentLat = mission.currentLat;
    mission.incidentCurrentLng = mission.currentLng;
  ...
  console.log(
    ...
    + `@ ${resolvedTargetLat},${resolvedTargetLng} (${initialStatus}, `
    + `${Math.round(
      distanceMeters(
        { lat: startLat, lng: startLng },
        { lat: resolvedTargetLat, lng: resolvedTargetLng },
  ...
  publishLiveGpsPosition({
    ...
    heading: pointAtDistance(mission.pathCoordinates, 0).heading || mission.lastHeading || 0,
  ...
  const replacementFields = {
    replacementMissionType: incident?.replacementMissionType
      || mission.replacementMissionType
      || null,
    replacementTargetLat: incident?.replacementTargetLat ?? mission.replacementTargetLat,
    replacementTargetLng: incident?.replacementTargetLng ?? mission.replacementTargetLng,
    replacementTargetStationName: incident?.replacementTargetStationName
      || mission.replacementTargetStationName
      || null,
    ...
    replacementEstimatedResumeAt: incident?.replacementEstimatedResumeAt
      ...
      || null,
    onboardPassengerCount: incident?.onboardPassengerCount ?? mission.onboardPassengerCount,
    futurePassengerCount: incident?.futurePassengerCount ?? mission.futurePassengerCount,
  };
...
/**
 * Điều tàu thay khách theo replacementMissionType.
 * - TransferAtIncidentLocation: ra hiện trường (mặc định nếu có replacementBoatCode)
 * - ContinueFromStation: tới bến target (khách chờ)
 * - PassengerRecoveryRequired: không auto — chờ ops
 * - None: chỉ SOS kéo, không điều tàu thay
 */
function startReplacementBoatAutomation(rescueMission, { incidentLat, incidentLng } = {}) {
  ...
  if (!missionType) missionType = 'TransferAtIncidentLocation';

  if (missionType === 'None') {
    console.log(`[rescue-gps] replacement None — chỉ SOS ${rescueMission.rescueBoatCode}`);
    return { started: false, skipped: true, reason: 'None', missionType };
  }
  if (missionType === 'PassengerRecoveryRequired') {
    console.warn(
      `[rescue-gps] PassengerRecoveryRequired — không auto điều ${replacementBoatCode}; cần ops`,
    );
    return { started: false, skipped: true, reason: 'PassengerRecoveryRequired', missionType };
  }

  let targetLat = Number(incidentLat);
  let targetLng = Number(incidentLng);
  if (missionType === 'ContinueFromStation') {
    const sLat = Number(rescueMission.replacementTargetLat);
    const sLng = Number(rescueMission.replacementTargetLng);
    if (Number.isFinite(sLat) && Number.isFinite(sLng)) {
      targetLat = sLat;
      targetLng = sLng;
    } else {
      const station = findStationForReplacementTarget(rescueMission);
      if (station) {
        targetLat = Number(station.lat);
        targetLng = Number(station.lng);
      } else {
        console.warn(
          `[rescue-gps] ContinueFromStation thiếu tọa độ bến `
          + `(${rescueMission.replacementTargetStationName || rescueMission.replacementTargetStationId}) `
          + `— fallback hiện trường`,
        );
      }
    }
  }
  ...
  return startRescueAutomation({
    incidentId: `${rescueMission.incidentId}__xfer`,
    boatCode: null,
    rescueBoatCode: replacementBoatCode,
...
function findStationForReplacementTarget(mission) {
  ...
  if (id) {
    const byId = stations.find((s) => (
      String(s.stationId || s.id || '').trim() === id
  ...
  if (name) {
    const byName = stations.find((s) => {
      const n = String(s.stationName || s.name || '').trim().toLowerCase();
      const c = String(s.stationCode || s.code || '').trim().toLowerCase();
      return n === name || n.includes(name) || c === name;
    });
    if (byName && Number.isFinite(Number(byName.lat)) && Number.isFinite(Number(byName.lng))) return byName;
  }
  return null;
}
...
async function tickRescueMissions() {
  ...
  await Promise.all(active.map(async (mission) => {
    ...
    try {
      ...
      // Điểm cuối path đã được chiếu lên corridor; không nhảy marker vào tọa độ bờ của bến.
      const publishLat = lat;
      const publishLng = lng;
      ...
      if (mission.incidentBoatCode) {
        if (mission.status === 'Towing') {
          // Khi kéo: tàu lỗi đi sau tàu cứu. Cập bến: cả hai tụ cùng điểm trên sông.
          ...
          const towedPosition = arrived ? lead : pointAtDistance(
            mission.pathCoordinates,
            Math.max(0, Number(mission.pathProgressMeters || 0) - towRopeMeters),
          );
      ...
      if (!arrived) {
        ...
        // Giữ đích = hiện trường đã pin lúc START.
        // Không bám hub tàu sự cố: trip GPS có thể vẫn đẩy SC → SOS đuổi mãi / đứng sai chỗ.
      } else if (mission.status === 'Towing') {
        mission.status = 'AtStation';
        mission.stationArrivedAt = mission.updatedAt;
        // Giữ điểm cuối corridor và gom hai tàu tại cùng tọa độ nước.
        const finalBerthLat = Number(publishLat);
        const finalBerthLng = Number(publishLng);
        ...
        if (Number.isFinite(finalBerthLat) && Number.isFinite(finalBerthLng)) {
          ...
          if (mission.incidentBoatCode) {
            mission.incidentCurrentLat = finalBerthLat;
            mission.incidentCurrentLng = finalBerthLng;
            await publishLiveGpsPosition({
              boatCode: mission.incidentBoatCode,
              lat: finalBerthLat,
              lng: finalBerthLng,
        ...
        if (towDest) {
          ...
          if (towDest.meters <= arrivalMeters) {
            // Đã sát bến → gom vào điểm corridor gần bến, tick sau nhả SOS.
            assignRescueRiverPath(
              mission,
              { lat: towDest.lat, lng: towDest.lng },
              { lat: towDest.lat, lng: towDest.lng },
            );
            const riverDock = mission.pathCoordinates.at(-1);
            if (riverDock) {
              mission.currentLat = riverDock.lat;
              mission.currentLng = riverDock.lng;
              if (mission.incidentBoatCode) {
                mission.incidentCurrentLat = riverDock.lat;
                mission.incidentCurrentLng = riverDock.lng;
              }
            }
...
function ingestIncidentHook(body = {}, req = null) {
  ...
  const incident = {
    ...
    replacementBoatCode: replacementBoatCode && replacementBoatCode !== rescueBoatCode
      ? replacementBoatCode
      : (replacementBoatCode || null),
    ...extractReplacementMissionFields(body),
    lat: Number.isFinite(lat) ? lat : null,
    lng: Number.isFinite(lng) ? lng : null,
...
/**
 * Charter: KHÔNG dùng from-stations. Lưu như GPS thường (from-gps).
 * Chặng cuối nhiều bến → tạo 1 route tổng via from-gps (mã riêng, có geometry).
 */
async function composeCharterRouteFromGps(session, body) {
  const allStops = Array.isArray(body.charterAllStops) ? body.charterAllStops : [];
  if (allStops.length < 2) {
    return { ok: false, status: 400, error: 'Thiếu charterAllStops để tạo route tổng.' };
  }
  ...
  const names = allStops
    ...
    .filter(Boolean);
  const coords = stitchCharterFullCoordinates(allStops, body, session);
  if (coords.length < 2) {
    return {
      ok: false,
      status: 400,
      error: 'Không ghép được geometry đủ các chặng để from-gps. Cần các chặng 2 bến đã lưu có path.',
    };
  }
  let routeCode = `${baseCode}-FULL`.slice(0, 40);
  const composeBody = {
    ...body,
    routeCode,
    routeName: cleanOptionalText(body.charterComposeName)
      || (names.length >= 2 ? `${names[0]} - ${names[names.length - 1]}` : routeCode),
    stops: allStops,
    startStationId: allStops[0]?.stationId || null,
    endStationId: allStops[allStops.length - 1]?.stationId || null,
    coordinates: coords,
    createReverseRoute: false,
    charterFinalLeg: false, // tránh đệ quy complete trong save
  };
  let result = await saveRouteFromGpsOnTarget(session, composeBody);
  if (isRouteCodeExistsError(result)) {
    routeCode = `${baseCode}-F${Date.now() % 100000}`.slice(0, 40);
    composeBody.routeCode = routeCode;
    result = await saveRouteFromGpsOnTarget(session, composeBody);
  }
  return result;
}

function isRouteCodeExistsError(result) {
  if (!result || result.ok) return false;
  if (result.status === 409) return true;
  const msg = String(result.error || '').toLowerCase();
  return msg.includes('already exists') || msg.includes('đã tồn tại') || msg.includes('routecode');
}

/** Ghép geometry các chặng 2 bến (DB) + path chặng vừa vẽ. */
function stitchCharterFullCoordinates(allStops, body, session) {
  const ordered = Array.isArray(allStops) ? allStops : [];
  if (ordered.length < 2) return [];
  const stitched = [];
  for (let i = 0; i < ordered.length - 1; i += 1) {
    const from = ordered[i];
    const to = ordered[i + 1];
    let seg = findLocalRouteCoordsBetweenStations(from, to);
    // Chặng cuối: ưu tiên path vừa ghi.
    if ((!seg || seg.length < 2) && i === ordered.length - 2) {
      seg = resolveSurveyPathCoordinates(session, body, Number(body.averageSpeedKmh) || 20);
    }
    if (!seg || seg.length < 2) continue;
    if (!stitched.length) stitched.push(...seg);
    else stitched.push(...seg.slice(1));
  }
  if (stitched.length >= 2) return stitched;
  return resolveSurveyPathCoordinates(session, body, Number(body.averageSpeedKmh) || 20);
}

function findLocalRouteCoordsBetweenStations(fromStop, toStop) {
  const routes = state.routes instanceof Map
    ? [...state.routes.values()]
  ...
  for (const route of routes) {
    ...
    if (stops.length === 2) {
      ...
      const same = (x, y) => {
        if (!x || !y) return false;
        if (String(x.stationId || '') && String(x.stationId) === String(y.stationId || '')) return true;
        const nx = String(x.stationCode || '').replace(/^ST-/i, '').toUpperCase();
        const ny = String(y.stationCode || '').replace(/^ST-/i, '').toUpperCase();
        return Boolean(nx && ny && nx === ny);
      };
      if (!same(a, fromStop) || !same(b, toStop)) continue;
      const coords = Array.isArray(route.coordinates) ? route.coordinates : [];
      if (coords.length < 2) continue;
      const score = coords.length;
      if (!best || score < best.score) {
        best = {
          score,
          coords: coords.map((p) => ({ lat: Number(p.lat), lng: Number(p.lng) }))
...
async function persistRecordingSession(body, sessionInput = null) {
  ...
  // Mỗi lần lưu chặng charter = đúng 2 bến (như GPS thường). Không gửi cả chuỗi CB.
  if (isCharter && body.charterFinalLeg !== true) {
    const startId = cleanOptionalText(body.startStationId || session?.startStationId);
    const endId = cleanOptionalText(body.endStationId || session?.endStationId);
    const rawStops = Array.isArray(body.stops) ? body.stops : (Array.isArray(session?.stops) ? session.stops : []);
    if (startId && endId && startId !== endId) {
      const byId = new Map(rawStops.map((s) => [String(s.stationId || ''), s]));
      const fromCat = (id) => (state.stations || []).find((s) => String(s.stationId) === String(id));
      const a = byId.get(String(startId)) || fromCat(startId) || { stationId: startId };
      const b = byId.get(String(endId)) || fromCat(endId) || { stationId: endId };
      body = {
        ...body,
        ...
        stops: [
          ...
          {
            stationId: endId,
            stationCode: b.stationCode || null,
            ...
            isPickupAllowed: true,
            isDropoffAllowed: true,
            standardTravelMin: rawStops.find((s) => String(s.stationId) === String(endId))?.standardTravelMin
              ?? null,
          },
        ],
      };
  ...
  // Charter: từng chặng lưu via from-gps như survey thường (không from-stations).
  const canTryAzure = Boolean(getTargetApiRoot() && hasCoordinates && (
    isCharter || session?.targetSessionStarted
  ));

  if (canTryAzure) {
    // Charter từng chặng: from-gps. Mã trùng → đổi mã rồi thử lại.
    let targetSave = await saveRouteFromGpsOnTarget(session, body);
    if (isCharter && isRouteCodeExistsError(targetSave)) {
      const base = String(body.routeCode || session?.routeCode || 'CHARTER').replace(/-C\d+$/i, '');
      const retryCode = `${base}-C${Date.now() % 100000}`.slice(0, 40);
      console.warn(`[charter] routeCode trùng → thử lại ${retryCode}`);
      targetSave = await saveRouteFromGpsOnTarget(
        { ...session, routeCode: retryCode },
        { ...body, routeCode: retryCode },
      );
      if (targetSave.ok) body = { ...body, routeCode: retryCode };
    }
