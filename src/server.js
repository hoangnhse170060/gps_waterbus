  ...
  try {
    ...
    if (response.status === 409) {
      ...
      if (!ok && retried.soft) {
        ...
        // Cập nhật hubBoats để map hiện tàu dù BE reject (sequence conflict).
        upsertHubBoat({
          boatCode: payload.boatCode,
          lat: payload.lat,
          lng: payload.lng,
          ...
          fromAzure: false,
          source: 'live-gps-sequence-conflict',
        });
        return {
          ok: true,
          soft: true,
      ...
      // Cập nhật hubBoats để map hiện tàu khi BE từ chối (sequence conflict / lỗi khác).
      upsertHubBoat({
        boatCode: payload.boatCode,
        lat: payload.lat,
        lng: payload.lng,
    ...
    if (!ok && statusCode === 400 && fromTrip && (
      ...
      for (const stripped of retryBodies) {
        ...
        try {
          ...
          // Retry thành công → cập nhật hubBoats để map hiện tàu (Azure BE không push boatLocation).
          upsertHubBoat({
            boatCode: stripped.boatCode,
            lat: stripped.lat,
            lng: stripped.lng,
            speedKmh: stripped.speedKmh,
            heading: stripped.heading,
            fromAzure: false,
            source: 'live-gps-retry',
          });
          return {
            ok: true,
            soft: true,
    ...
    // Cập nhật hubBoats để map luôn hiện tàu — dù Azure trả gì (200/400/409/error).
    upsertHubBoat({
      boatCode: payload.boatCode,
      lat: payload.lat,
      lng: payload.lng,
      ...
      heading: payload.heading,
      fromAzure: false,
      source: 'live-gps-' + (ok ? 'success' : 'fail'),
    });
    return {
      ok,
      status: statusCode,
    ...
    // Network error — vẫn cập nhật hubBoats để map hiện tàu.
    upsertHubBoat({
      boatCode: payload.boatCode,
      lat: payload.lat,
      lng: payload.lng,
